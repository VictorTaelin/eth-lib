...
var Api = function Api(provider) {
  ...
  var addTransactionDefaults = function addTransactionDefaults(tx) {
    return Promise.all([tx.chainId || send("net_version")(), tx.gasPrice || send("eth_gasPrice")(), tx.nonce || send("eth_getTransactionCount")(tx.from, "latest"), tx.value || "0x0", tx.data || "0x"]).then(function (_ref) {
      ...
      return send("eth_estimateGas")(removeEmptyTo(tx)).then(function (usedGas) {
        return Map.merge(tx)({
